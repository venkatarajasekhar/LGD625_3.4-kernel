...
#include "lge/lge_handle_panic.h"
#include "board.h"
#endif
...
unsigned long modem_info_ctx_reserved;
#endif
#if defined(CONFIG_CPU_CP15_MMU) && defined(CONFIG_LGE_HANDLE_PANIC_CP15)
unsigned long cpu_crash_ctx_reserved;
#endif
...
void tegra_assert_system_reset(char mode, const char *cmd)
{
	...
	do { } while (1);
#else
	static void __iomem *reset = IO_ADDRESS(TEGRA_PMC_BASE + 0);
	static u32 reg;

	printk("[POWER] tegra_assert_system_reset!! cmd=%s\n",cmd);
	...
	if (cmd) {
		if (!strcmp(cmd, "recovery")){
			lge_set_restart_reason(0x77665502);
			reg |= RECOVERY_MODE;
		}
		else if (!strcmp(cmd, "bootloader")){
			lge_set_restart_reason(0x77665500);
			reg |= BOOTLOADER_MODE;
		}
        //                                                                    
        else if (!strcmp(cmd, "--bnr_recovery")){
            lge_set_restart_reason(0x77665555);
            reg |= RECOVERY_MODE;
        }
        //                                                                    
			...
			lge_set_restart_reason(0x6C616664);
			reg &= ~(BOOTLOADER_MODE | RECOVERY_MODE | FORCED_RECOVERY_MODE);
        }else if(!strcmp(cmd, "poweroff")){
			printk("[POWER] set restart reason 10041004!!!\n");
			lge_set_restart_reason(0x10041004);
			reg &= ~(BOOTLOADER_MODE | RECOVERY_MODE | FORCED_RECOVERY_MODE);
        }
		else
		{
...
#ifdef CONFIG_MACH_PDA
		// dump_stack();
		lge_set_restart_reason(0x77665503);
#endif
	}
...
{
#ifdef CONFIG_MACH_PDA
	/* PMIC EN5 of PD_A Rev.B control scheme was changed */
	/* SL440 chip will be used since PD_A Rev.B board    */
	if (tegra_sku_id == 0x7) {
		pr_info("modem_config(1) was changed by T4i SKU\n");
		modem_config = 0x1;
	}
#endif
	return modem_config;
...
#ifdef CONFIG_MACH_PDA
static int hw_revision;
static int pcb_revision;

struct hw_revision_table {
	int mv;
	int rev;
};

struct hw_revision_table hw_rev_table[] = {
	{ 175, 0xA }, /*   A: 100K, 6.2K,  0.11V */
	{ 260, 0xB }, /*   B: 100K, 12.1K, 0.19V */
	{ 360, 0xC }, /*   C: 100K, 20K,   0.30V */
	{ 450, 0xD }, /*   D: 100K, 27K,   0.38V */
	{ 570, 0xE }, /*   E: 100K, 39K,   0.51V */
	{ 800, 0xF }, /*   F: 100K, 51K,   0.61V */
	{1000, 0x10}, /* 1.0: 100K, 100K,  0.90V */
	{1180, 0x11}, /* 1.1: 100K, 150K,  1.10V */
	{1300, 0x12}, /* 1.2: 100K, 215K,  1.23V */
};
int tegra_get_pcb_rev(void)
{
	return pcb_revision;
}

int tegra_get_hw_revision(void)
{
	return hw_revision;
}

static int __init tegra_hw_revision(char *id)
{
	char *p = id;
	int i, count;

	hw_revision = memparse(p, &p);

	count = sizeof(hw_rev_table)/sizeof(struct hw_revision_table);
	for (i = 0; i < count; i++) {
		if (hw_revision < hw_rev_table[i].mv) {
			pcb_revision = hw_rev_table[i].rev;
			break;
		}
	}
	if (pcb_revision == 0xA && tegra_sku_id == 0x7) {
		pcb_revision = 0xB; /* correct PCB version for Rev.B */
	}
	pr_info("%s: PD_A PCB Rev.%X (%d mv)\n",
		__func__, pcb_revision, hw_revision);

	return 1;
}

__setup("hw_revision=", tegra_hw_revision);
#endif /* CONFIG_MACH_PD_A */
...
#endif
#if defined(CONFIG_CPU_CP15_MMU) && defined(CONFIG_LGE_HANDLE_PANIC_CP15)
	cpu_crash_ctx_reserved = 0xb7401000;
	if (cpu_crash_ctx_reserved) {
		if (memblock_reserve(cpu_crash_ctx_reserved, CPU_CRASH_CTX_SIZE)) {
			pr_err("Failed to reserve cpu_crash_ctx page %08lx@%08lx\n",
			       cpu_crash_ctx_reserved, CPU_CRASH_CTX_SIZE);
			cpu_crash_ctx_reserved = 0;
		}
	}
#endif
...
#ifdef CONFIG_TEGRA_NVDUMPER
#ifdef CONFIG_MACH_PDA
	modem_info_ctx_reserved = 0xb7402000;
	if (modem_info_ctx_reserved) {
		if (memblock_reserve(modem_info_ctx_reserved, MODEM_CTX_RESERVED_SIZE)) {
			pr_err("Failed to reserve nvdumper page %08lx@%08lx\n",
				   modem_info_ctx_reserved, MODEM_CTX_RESERVED_SIZE);
			modem_info_ctx_reserved = 0;
		}
	}
#endif
#endif
