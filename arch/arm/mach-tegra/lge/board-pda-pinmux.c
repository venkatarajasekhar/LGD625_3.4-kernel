...
static __initdata struct tegra_drive_pingroup_config pda_drive_pinmux[] = {
...
#include "board-pda-pinmux-t14x.h"
#include "board-atlantis-pda-pinmux-t14x.h"

static void __init pda_gpio_init_configure(void)
{
	int len;
	int i;
...
/*                                                                                            
	...
	if(lge_get_board_revno() == HW_REV_PDA_B) {
		len = ARRAY_SIZE(init_gpio_mode_pda_common_rev_b);
		pins_info = init_gpio_mode_pda_common_rev_b;
	}
	else if(lge_get_board_revno() == HW_REV_PDA_C) {
		len = ARRAY_SIZE(init_gpio_mode_pda_common_rev_c);
		pins_info = init_gpio_mode_pda_common_rev_c;
	}
	else {
		len = ARRAY_SIZE(init_gpio_mode_pda_common_rev_c);
		pins_info = init_gpio_mode_pda_common_rev_c;
...
int __init pda_pinmux_init(void)
{
...
/*                                                                                            
                                                                                             ...
                                                                                             */
		if(lge_get_board_revno() == HW_REV_PDA_B){
			tegra_pinmux_config_table(pda_pinmux_common_rev_b,
			ARRAY_SIZE(pda_pinmux_common_rev_b));
		}
		else if(lge_get_board_revno() >= HW_REV_PDA_C) {
			tegra_pinmux_config_table(pda_pinmux_common_rev_c,
			ARRAY_SIZE(pda_pinmux_common_rev_c));
		}
		break;
	case BOARD_E1670:
	case BOARD_E1671:
	...
	tegra_drive_pinmux_config_table(pda_drive_pinmux,
					ARRAY_SIZE(pda_drive_pinmux));
	...
	case BOARD_E1683:
	case BOARD_E1690:
		tegra_pinmux_config_table(pda_unused_pins_lowpower,
			ARRAY_SIZE(pda_unused_pins_lowpower));
		break;
	case BOARD_E1670:
	case BOARD_E1671:
	case BOARD_E1740:
...
static struct gpio_init_pin_info pda_suspend_pin_state[] = {
...
int pda_pinmux_suspend(void)
{
	...
	case BOARD_E1683:
	case BOARD_E1690:
		pins_info = pda_suspend_pin_state;
#ifdef CONFIG_MACH_PDA
		len = 0;
#else
		len = ARRAY_SIZE(pda_suspend_pin_state);
#endif
		break;
	case BOARD_E1670:
	case BOARD_E1671:
