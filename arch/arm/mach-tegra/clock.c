...
static int __init tegra_clk_late_init(void)
{
	...
	if ( lge_get_boot_mode() == LGE_BOOT_MODE_QEM_56K || \
		...
		pm_qos_add_request(&cpufreq_min_req, PM_QOS_CPU_FREQ_MAX,
				   PM_QOS_CPU_FREQ_MAX_DEFAULT_VALUE);

		/* limit max freq due to usb port failure on minOS */
		/* 51000 102000 204000 310000 310000 398000 492000 565000 657000
		   729600 816000 918000 1020000 1122000 1224000 1326000 1428000
		   1530000 1632000 1734000 */
		pm_qos_update_request_timeout(&cpufreq_min_req, 1020000, (unsigned long)(-1));
		printk("limit max freq to 657Mhz due to usb port failure on factory mode\n");
	}
	else {
		pm_qos_add_request(&cpufreq_min_req, PM_QOS_CPU_FREQ_MIN,
				   PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);

		/* Boost cpu freq 1734MHz for 20s */
		pm_qos_update_request_timeout(&cpufreq_min_req, 1734000, 20000000);
		printk("boost cpu freq 1.7Ghz for 20s\n");
	}
#endif
