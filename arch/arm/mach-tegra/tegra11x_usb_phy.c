...
static unsigned int utmi_phy_xcvr_setup_value(struct tegra_usb_phy *phy)
{
	...
	if(lge_get_factory_boot())
		cfg->xcvr_setup_offset = -10;

	if (cfg->xcvr_use_fuses) {
		val = XCVR_SETUP(tegra_fuse_readl(FUSE_USB_CALIB_0));

		pr_info("##@%s fuse usb calib(%ld)\n", __func__, val);
		if (val + cfg->xcvr_setup_offset < 0)
			pr_info("##@%s ERR: -xcvr_setup_offset(%d) shoud be bigger than -%ld\n",
					__func__, cfg->xcvr_setup_offset, val);
...
static int utmi_phy_power_on(struct tegra_usb_phy *phy)
{
	...
	{
		val = readl(base + UTMIP_XCVR_CFG0);
		printk("##@%s UTMIP_XCVR_CFG0 val(0x%8lx)\n", __func__, val);
		val &= 0x003FFFF0;
		val |= 0x02000000; // XCVR_HSSLEW_MSB:1
		val |= 0x01C00000; // XCVR_HSSETUP_MSB:7
		val |= 0x0000000F; // XCVR_SETUP:F
		writel(val, base + UTMIP_XCVR_CFG0);

		val = readl(base + UTMIP_XCVR_CFG0);
		printk("##@%s after UTMIP_XCVR_CFG0 val(0x%8lx)\n", __func__, val);

#define UTMIPB_BIAS_CFG0	0x80C
		val = readl(base + UTMIPB_BIAS_CFG0);
		printk("##@%s UTMIP_BIAS_CFG0 val(0x%8lx)\n", __func__, val);

		val &= 0xFFFFFFFC;
		val |= 0x00000003; // HSSQUELCH_LEVEL: 3
		writel(val, base + UTMIPB_BIAS_CFG0);

		val = readl(base + UTMIPB_BIAS_CFG0);
		printk("##@%s after UTMIP_BIAS_CFG0 val(0x%8lx)\n", __func__, val);
	}
#endif
