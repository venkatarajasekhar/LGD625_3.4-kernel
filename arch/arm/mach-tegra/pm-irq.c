...
#define PMC_ALLOW_PULSE_WAKE	(1ull << 63)
...
static u64 tegra_lp0_wake_edge;
...
bool is_max77660_pmic_wakeup;
EXPORT_SYMBOL(is_max77660_pmic_wakeup);
...
static void tegra_pm_irq_syscore_resume(void)
{
	...
	/* clear PMC_ALLOW_PULSE_WAKE bit to use the wake level */
	if (tegra_lp0_wake_edge & tegra_lp0_wake_enb) {
		u64 wake_level = read_pmc_wake_level();
		wake_level &= ~PMC_ALLOW_PULSE_WAKE;
