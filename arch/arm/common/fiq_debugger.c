...
#include <../mach-tegra/lge/lge_handle_panic.h>
#endif
...
static void dump_allregs(struct fiq_debugger_state *state, unsigned *regs)
{
	...
	debug_printf(state, " fiq: sp %08x  lr %08x  spsr %08x\n",
...
#ifdef CONFIG_CPU_CP15
       {
               ...
               debug_printf(state, "Control: %08x%s\n", ctrl);

#if defined(CONFIG_CPU_CP15_MMU) && defined(CONFIG_LGE_HANDLE_PANIC_CP15)
               lge_save_ctx((struct pt_regs*)regs, ctrl, c1, c2);
#ifdef CONFIG_MACH_PDA
...
#ifdef CONFIG_TEGRA_NVDUMPER
	nvdumper_copy_regs(this_cpu, regs, svc_sp);
#endif /* CONFIG_TEGRA_NVDUMPER */
...
#ifdef CONFIG_MACH_PDA
       if(!pm_power_reset) {
               void __iomem *reset = IO_ADDRESS(0x7000E400);
               u32 reg;
...
static int fiq_debugger_probe(struct platform_device *pdev)
{
	...
	pr_err("<hit enter %sto activate fiq debugger>\n",
