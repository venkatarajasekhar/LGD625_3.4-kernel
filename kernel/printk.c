	...
	for (; *p; p++) {
		if (new_text_line) {
			...
			if (printk_time) {
				...
				tlen = snprintf(tbuf, 255, "[%5lu.%06lu %02d-%02d %02d:%02d:%02d.%03lu] ", /* WBT_TD2170540019 */
										(unsigned long) t,
				                                                nanosec_rem / 1000,
								tmresult.tm_mon + 1,
								tmresult.tm_mday,
				...
				t = cpu_clock(printk_cpu);
				nanosec_rem = do_div(t, 1000000000);
				tlen = sprintf(tbuf, 49, "[%5lu.%06lu] ", /* WBT_TD2170540019 */
						(unsigned long) t,
						nanosec_rem / 1000);
#endif
