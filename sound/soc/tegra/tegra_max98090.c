...
#elif defined(CONFIG_FM_RADIO)
#define DAI_LINK_FM_RADIO		4
#endif
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = machine->is_fm_mode;
...
{
	struct tegra_max98090 *machine = snd_kcontrol_chip(kcontrol);
	int is_fm_mode_new = ucontrol->value.integer.value[0];
	static struct timespec time;
#if defined(CONFIG_FM_RADIO_LP1_STATE)
	int codec_index = HIFI_CODEC;
	int i;
#endif

	if (machine->is_fm_mode == is_fm_mode_new) {
		pr_err("%s: last connection info : [%s] - %2d:%02d:%02d\n",
						__func__,
						is_fm_mode_new ? "CONNECT":"DISCONNECT",
						machine->fm_time.tm_hour,
						machine->fm_time.tm_min,
						machine->fm_time.tm_sec);
		return 0;
	} else {
		time = __current_kernel_time();
		time_to_tm(time.tv_sec, sys_tz.tz_minuteswest * 60 * (-1), &machine->fm_time);
	}
	...
	if (is_fm_mode_new) {
		...
		for (i = 0; i < machine->pcard->num_links; i++)
			machine->pcard->dai_link[i].ignore_suspend = 1;
#else
		wake_lock(&machine->fm_wakelock);
#endif
		make_fm_radio_connections(
			 &machine->codec_info[HIFI_CODEC],
			 &machine->codec_info[FM_RADIO]);
	} else {
		wake_unlock(&machine->fm_wakelock);
		break_fm_radio_connections(
				&machine->codec_info[HIFI_CODEC],
				&machine->codec_info[FM_RADIO]);
#if defined(CONFIG_FM_RADIO_LP1_STATE)
		for (i = 0; i < machine->pcard->num_links; i++)
	...
	machine->is_fm_mode = is_fm_mode_new;
#if defined(CONFIG_FM_RADIO_LP1_STATE)
	g_is_call_mode = machine->is_fm_mode;
#endif
	return 1;
}

static int tegra_fm_gain_info(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 1;
...
static int tegra_fm_gain_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct tegra_max98090 *machine = snd_kcontrol_chip(kcontrol);
	ucontrol->value.integer.value[0] = machine->fm_volume;

	return 1;
}

static int tegra_fm_gain_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct tegra_max98090 *machine = snd_kcontrol_chip(kcontrol);
	int requested_fm_volume = ucontrol->value.integer.value[0];

	machine->fm_volume = requested_fm_volume;

	if (machine->is_fm_mode) {
		adjust_fm_radio_volume(&machine->codec_info[FM_RADIO], requested_fm_volume);
	} else {
		pr_info("%s->fm disabled!\n", __func__);
	}
	return 1;
}
...
struct snd_kcontrol_new tegra_max98090_fm_gain_control = {
	.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.name = "FM Volume",
	.private_value = 0xffff,
	.info = tegra_fm_gain_info,
	.get = tegra_fm_gain_get,
	.put = tegra_fm_gain_put
};
...
#ifdef CONFIG_FM_RADIO
		if (machine->is_fm_mode) {
			i2s->is_fm_mode_rec = machine->is_fm_mode;
			codec_info = &machine->codec_info[FM_RADIO];

			/* setup the connections for fm call record */
...
#ifdef CONFIG_FM_RADIO
		if (i2s->is_fm_mode_rec) {
			i2s->is_fm_mode_rec = 0;
			tegra30_ahub_unset_rx_cif_source(i2s->rxcif);
		} else {
#else
			if (!i2s->is_call_mode_rec)
...
static int tegra_max98090_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	for (i = 0; i < NUM_I2S_DEVICES; i++) {
		if (i2s->id == machine->codec_info[i].i2s_id) {
			i2s->boost_cpu_freq =
				pdata->i2s_param[i].boost_cpu_freq;
			i2s->boost_emc_freq =
				pdata->i2s_param[i].boost_emc_freq;
			i2s->boost_sclk_freq =
				pdata->i2s_param[i].boost_sclk_freq;
			break;
		}
	}
	...
	wake_lock_init(&machine->fm_wakelock, WAKE_LOCK_SUSPEND, "fmradio");
	...
	ret = snd_ctl_add(codec->card->snd_card,
		snd_ctl_new1(&tegra_max98090_fm_gain_control, machine));
	if (ret < 0)
		return ret;
#endif
...
static int tegra_max98090_suspend_post(struct snd_soc_card *card)
{
	...
	if (suspend_allowed) {
		...
		if (gpio_is_valid(machine->pdata->gpio_spkr_en) &&
				(lge_get_board_revno() >= HW_REV_PDA_C) &&
				!(machine->is_call_mode)) {
			gpio_direction_output(machine->pdata->gpio_spkr_en, 0);
			machine->gpio_requested &= ~GPIO_SPKR_EN;
			pr_info("%s: turn off SPK\n", __func__);
		}
#endif
...
static int tegra_max98090_resume_pre(struct snd_soc_card *card)
{
	...
	if (suspend_allowed) {
		...
		if (gpio_is_valid(machine->pdata->gpio_spkr_en) &&
				(lge_get_board_revno() >= HW_REV_PDA_C)) {
			gpio_direction_output(machine->pdata->gpio_spkr_en, 1);
			machine->gpio_requested |= GPIO_SPKR_EN;
			pr_info("%s: turn on SPK\n", __func__);
		}
#endif
...
/*                                                                                            
                                                                                             ...
                                                                                             */
	if (gpio_is_valid(pdata->gpio_spkr_en) && (lge_get_board_revno() >= HW_REV_PDA_C)) {
...
#if defined(CONFIG_FM_RADIO)
	wake_lock_destroy(&machine->fm_wakelock);
#endif
