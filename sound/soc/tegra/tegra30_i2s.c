...
static void tegra30_i2s_freq_boost_work(struct work_struct *work)
{
	struct tegra30_i2s *i2s = container_of(work, struct tegra30_i2s,
					freq_boost_work.work);

	pr_debug("%s: freq_boost_on %d\n", __func__, i2s->freq_boost_on);

	mutex_lock(&i2s->freq_boost_lock);
	if (i2s->freq_boost_on) {
		if (i2s->boost_cpu_freq > 0)
			pm_qos_update_request(&i2s->cpufreq_min_req,
					i2s->boost_cpu_freq / 1000);
		if (i2s->emc_clk && (i2s->boost_emc_freq > 0)) {
			clk_set_rate(i2s->emc_clk, i2s->boost_emc_freq);
			clk_prepare_enable(i2s->emc_clk);
		}
		if (i2s->sclk_clk && (i2s->boost_sclk_freq > 0)) {
			clk_set_rate(i2s->sclk_clk, i2s->boost_sclk_freq);
			clk_prepare_enable(i2s->sclk_clk);
		}
	} else {
		if (i2s->sclk_clk && (i2s->boost_sclk_freq > 0))
			clk_disable_unprepare(i2s->sclk_clk);
		if (i2s->emc_clk && (i2s->boost_emc_freq > 0))
			clk_disable_unprepare(i2s->emc_clk);
		if (i2s->boost_cpu_freq > 0)
			pm_qos_update_request(&i2s->cpufreq_min_req,
					PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);
	}
	mutex_unlock(&i2s->freq_boost_lock);
}
...
//Force Sample Rate Change For BT SCO VoIP
        if(i2s->id == 1 )  i2sclock = 2048000;
...
static void tegra30_i2s_start_capture(struct tegra30_i2s *i2s)
{
	...
	if (!i2s->is_call_mode_rec && !i2s->is_fm_mode_rec &&
				...
				struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		if (i2s->boost_cpu_freq > 0 ||
			i2s->boost_emc_freq > 0 || i2s->boost_sclk_freq > 0) {
			mutex_lock(&i2s->freq_boost_lock);
			i2s->freq_boost_on = true;
			mutex_unlock(&i2s->freq_boost_lock);
			schedule_delayed_work(&i2s->freq_boost_work, 1);
		}
...
static int configure_dam(struct tegra30_i2s  *i2s, int out_channel,
		...
		int in_rate, int in_bitsize,int fm_set)
{
	if (!i2s->dam_ch_refcount)
    ...
    if(fm_set == FM_ON)
	    tegra30_dam_set_gain(i2s->dam_ifc, TEGRA30_DAM_CHIN0_SRC, 0);
    else 
				...
				int uses_voice_codec)
{
	...
	ret = configure_dam(codec_i2s, codec_info->channels,
	   ...
	   bb_info->rate, bb_info->sample_size, FM_OFF);
	if (ret)
	...
	ret = configure_dam(bb_i2s, bb_info->channels,
		...
		codec_info->rate, codec_info->bitsize, FM_OFF);
	if (ret)
				...
				int uses_voice_codec)
{
	...
	ret = configure_dam(codec_i2s, bb_info->channels,
		bb_info->rate, bb_info->sample_size, codec_info->channels,
		48000, codec_info->bitsize, FM_OFF);
	if (ret)
		pr_info("%s:Failed to configure codec dam\n", __func__);
...
int adjust_fm_radio_volume(struct codec_config *config_info, int vol)
{
	struct tegra30_i2s *i2s = &i2scont[config_info->i2s_id];
	return tegra30_dam_set_gain(i2s->dam_ifc, TEGRA30_DAM_CHIN0_SRC, vol);
}

int make_fm_radio_connections(struct codec_config *codec_info,
				struct codec_config *fm_info)
{
	struct tegra30_i2s *codec_i2s = &i2scont[codec_info->i2s_id];
	struct tegra30_i2s *fm_i2s = &i2scont[fm_info->i2s_id];
	int dcnt = 10;
	int ret = 0;

	/* increment the codec i2s playback ref count */
	codec_i2s->playback_ref_count++;
	fm_i2s->capture_ref_count++;
	...
	/* Configure fm radio i2s */
	ret = configure_baseband_i2s(fm_i2s, fm_info->is_i2smaster,
		fm_info->i2s_mode, fm_info->channels,
		fm_info->rate, fm_info->bitsize,
		fm_info->bit_clk);
	if (ret)
		pr_info("%s:Failed to configure codec i2s\n", __func__);
	...
	/* configure codec i2s tx cif */
	tegra30_i2s_set_cif_channels(codec_i2s, TEGRA30_I2S_CIF_TX_CTRL,
			fm_info->channels, codec_info->channels);
	tegra30_i2s_set_cif_bits(codec_i2s, TEGRA30_I2S_CIF_TX_CTRL,
			TEGRA30_AUDIOCIF_BITS_16, TEGRA30_AUDIOCIF_BITS_16);

	tegra30_dam_ch1_set_datasync(codec_i2s->dam_ifc, 1);

	/* configure_dam allocate channel and setup SRC for CHIN0 */
	ret = configure_dam(codec_i2s, fm_info->channels,
		fm_info->rate, fm_info->bitsize, codec_info->channels,
		codec_info->rate, codec_info->bitsize, FM_ON);
	if (ret)
		pr_info("%s:Failed to configure codec dam\n", __func__);
	...
	fm_i2s->reg_ctrl |= TEGRA30_I2S_CTRL_XFER_EN_RX;
	tegra30_i2s_write(fm_i2s, TEGRA30_I2S_CTRL,
		fm_i2s->reg_ctrl);

	return 0;
}

int break_fm_radio_connections(struct codec_config *codec_info,
				struct codec_config *fm_info)
{
	struct tegra30_i2s *codec_i2s = &i2scont[codec_info->i2s_id];
	struct tegra30_i2s *fm_i2s = &i2scont[fm_info->i2s_id];
	...
	/* Disable FM radio I2X RX (TX to abub) */
	if (fm_i2s->capture_ref_count == 1) {
		fm_i2s->reg_ctrl &= ~TEGRA30_I2S_CTRL_XFER_EN_RX;

		tegra30_i2s_write(fm_i2s, TEGRA30_I2S_CTRL,
				fm_i2s->reg_ctrl);

		while (!tegra30_ahub_tx_fifo_is_empty(fm_i2s->id) &&
				dcnt--)
			udelay(100);
	...
	/* Soft reset */
	tegra30_i2s_soft_reset(codec_i2s);
	tegra30_i2s_soft_reset(fm_i2s);

	/* Disable the clocks */
	tegra30_i2s_disable_clocks(codec_i2s);
	tegra30_i2s_disable_clocks(fm_i2s);
				...
				int uses_voice_codec)
{
	...
	if (uses_voice_codec) {
		...
		configure_dam(codec_i2s, codec_info->channels,
		   ...
		   bb_info->rate, bb_info->bitsize, FM_OFF);
...
static __devinit int tegra30_i2s_platform_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&i2s->cpufreq_min_req, PM_QOS_CPU_FREQ_MIN,
			PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);

	i2s->emc_clk = clk_get(&pdev->dev, "emc");
	if (IS_ERR(i2s->emc_clk)) {
		dev_warn(&pdev->dev, "Can't retrieve emc clock\n");
		i2s->emc_clk = NULL;
	}

	i2s->sclk_clk = clk_get(&pdev->dev, "sclk");
	if (IS_ERR(i2s->sclk_clk)) {
		dev_warn(&pdev->dev, "Can't retrieve sclk clock\n");
		i2s->sclk_clk = NULL;
	}

	mutex_init(&i2s->freq_boost_lock);
	INIT_DELAYED_WORK(&i2s->freq_boost_work, tegra30_i2s_freq_boost_work);
...
err_unmap:
	if (i2s->sclk_clk)
		clk_put(i2s->sclk_clk);
	if (i2s->emc_clk)
		clk_put(i2s->emc_clk);
	pm_qos_remove_request(&i2s->cpufreq_min_req);
...
static int __devexit tegra30_i2s_platform_remove(struct platform_device *pdev)
{
	...
	flush_delayed_work_sync(&i2s->freq_boost_work);
	mutex_destroy(&i2s->freq_boost_lock);
	if (i2s->sclk_clk)
		clk_put(i2s->sclk_clk);
	if (i2s->emc_clk)
		clk_put(i2s->emc_clk);
