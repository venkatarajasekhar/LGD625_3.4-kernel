...
static int tegra_delay_get(struct snd_kcontrol *kcontrol,
         struct snd_ctl_elem_value *ucontrol)
{
    printk("%s - \n",__func__);
    return 0;
}
static int tegra_delay_put(struct snd_kcontrol *kcontrol,
        struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
    ...
    snd_soc_write(codec, M98090_REG_11_LVL_MIC2, gain);
    
    return 0;
}
struct snd_kcontrol_new tegra_max98090_lge_controls[] = {
    SOC_SINGLE_EXT("MAX98090 MIC MUTE",
            0,0,0,0,tegra_delay_get,tegra_delay_put),
...
//            val = max98090->extmic_mux + 1;
    ...
    printk("%s Event:%d Val: %d - %d\n",__func__,event,val,max98090->extmic_mux);
...
static int max98090_add_widgets(struct snd_soc_codec *codec)
{
    ...
    snd_soc_add_codec_controls(codec, tegra_max98090_lge_controls,
            ARRAY_SIZE(tegra_max98090_lge_controls));
...
//        usleep_range(300000, 500000);  //MYUNGWON.KIM, For MIC Bias Enable Noise
		break;
