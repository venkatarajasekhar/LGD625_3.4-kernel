...
struct tegra_uart_port {
	...
	/* freq boosting */
	struct pm_qos_request	cpufreq_min_req;
	unsigned int		boost_cpu_freq; /* Hz */
	struct clk		*emc_clk;
	unsigned int		boost_emc_freq; /* Hz */
	struct clk		*sclk_clk;
	unsigned int		boost_sclk_freq; /* Hz */
...
static void tegra_uart_hw_deinit(struct tegra_uart_port *t)
{
	...
	if (t->sclk_clk)
		clk_disable_unprepare(t->sclk_clk);
	if (t->emc_clk)
		clk_disable_unprepare(t->emc_clk);
	if (t->boost_cpu_freq > 0)
		pm_qos_update_request(&t->cpufreq_min_req,
...
static int tegra_uart_hw_init(struct tegra_uart_port *t)
{
	...
	if (t->boost_cpu_freq > 0)
		pm_qos_update_request(&t->cpufreq_min_req,
				t->boost_cpu_freq / 1000);
	if (t->emc_clk) {
		clk_set_rate(t->emc_clk, t->boost_emc_freq);
		clk_prepare_enable(t->emc_clk);
	}
	if (t->sclk_clk) {
		clk_set_rate(t->sclk_clk, t->boost_sclk_freq);
		clk_prepare_enable(t->sclk_clk);
	}
...
//				pdata->parent_clk_list[final_index].name);
		dev_info(t->uport.dev, "Setting clk_src %s, rate(%lu), parent_rate(%lu)\n",
				pdata->parent_clk_list[final_index].name, rate, parent_rate);
...
static int __init tegra_uart_probe(struct platform_device *pdev)
{
	...
	if (pdata->boost_cpu_freq > 0) {
		pm_qos_add_request(&t->cpufreq_min_req, PM_QOS_CPU_FREQ_MIN,
				   PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);
		t->boost_cpu_freq = pdata->boost_cpu_freq;
	}

	if (pdata->boost_emc_freq > 0) {
		t->emc_clk = clk_get(&pdev->dev, "emc");
		if (IS_ERR_OR_NULL(t->emc_clk)) {
			dev_err(&pdev->dev, "Couldn't get the emc clock\n");
			ret = -ENODEV;
			goto fail;
		}
		t->boost_emc_freq = pdata->boost_emc_freq;
	}

	if (pdata->boost_sclk_freq > 0) {
		t->sclk_clk = clk_get(&pdev->dev, "sclk");
		if (IS_ERR_OR_NULL(t->sclk_clk)) {
			dev_err(&pdev->dev, "Couldn't get the sclk clock\n");
			ret = -ENODEV;
			goto fail;
		}
		t->boost_sclk_freq = pdata->boost_sclk_freq;
	}
...
fail:
	...
	if (t->emc_clk)
		clk_put(t->emc_clk);
	if (t->boost_cpu_freq > 0)
		pm_qos_remove_request(&t->cpufreq_min_req);
	if (t->clk)
		clk_put(t->clk);
