...
static void bcm2079x_disable_irq(struct bcm2079x_dev *bcm2079x_dev)
{
    ...
    if (bcm2079x_dev->irq_enabled) {
		...
		printk("bcm2079x_disable_irq (disable_irq_nosync) \n");
        if (bcm2079x_dev->irq_wake_enabled) {
            disable_irq_wake(bcm2079x_dev->client->irq);
		 printk("bcm2079x_disable_irq (disable_irq_wake) \n");
...
static void bcm2079x_enable_irq(struct bcm2079x_dev *bcm2079x_dev)
{
    ...
    if (!bcm2079x_dev->irq_enabled) {
        bcm2079x_dev->irq_enabled = true;
		 ...
		 printk("bcm2079x_enable_irq (enable_irq) \n");
        if (!bcm2079x_dev->irq_wake_enabled) {
               enable_irq_wake(bcm2079x_dev->client->irq);
		  printk("bcm2079x_enable_irq (enable_irq_wake) \n");
...
printk("bcm2079x_dev_open  bcm2079x_dev->client->irq : %d\n", bcm2079x_dev->client->irq);
...
//      platform_data = kzalloc(sizeof(struct bcm2079x_platform_data), GFP_KERNEL);

	printk("bcm2079x_probe client->irq : %d \n",client->irq);


	if(client->dev.platform_data){
	    platform_data = (struct bcm2079x_platform_data *)client->dev.platform_data;
	}
	else{
		dev_err(&client->dev, "platform data  fail\n");
		return -ENODEV;
	}
	printk("bcm2079x_probe irq: %d, en : %d, wake : %d \n",platform_data->irq_gpio, platform_data->en_gpio, platform_data->wake_gpio);



	ret = gpio_request(platform_data->irq_gpio, "NFC_INT");
	if (ret) {
		pr_err("%s: failed to request NFC_INT%d\n", __func__, platform_data->irq_gpio);
		goto err_en;
	}
	...
	if (ret) {
		pr_err("%s: failed to request en_gpio%d\n", __func__, platform_data->en_gpio);
		goto err_wake;
	}
	...
	if (ret) {
		pr_err("%s: failed to request wake_gpio%d\n", __func__, platform_data->wake_gpio);
		goto err_exit;
	}
