...
#define EXT_LO_BIT			BIT(3)
#define EXT_HI_BIT			BIT(4)
#define LOC_LO_BIT			BIT(5)
#define LOC_HI_BIT			BIT(6)
...
struct nct1008_data {
	...
	long current_local_lo_limit;
	long current_local_hi_limit;
	int conv_period_ms;
	long etemp;
	int nct_disabled;
	int stop_workqueue;
...
static int nct1008_thermal_set_limits(struct thermal_zone_device *thz,
				      ...
				      long hi_limit_milli)
{
	...
	long *current_lo_limit, *current_hi_limit;
	u8 reg_lo, reg_hi;

	if (thz->ops == &nct_int_ops) {
		current_lo_limit = &data->current_local_lo_limit;
		current_hi_limit = &data->current_local_hi_limit;
		reg_lo = LOCAL_TEMP_LO_LIMIT_WR;
		reg_hi = LOCAL_TEMP_HI_LIMIT_WR;
	} else if (thz->ops == &nct_ext_ops) {
		current_lo_limit = &data->current_lo_limit;
		current_hi_limit = &data->current_hi_limit;
		reg_lo = EXT_TEMP_LO_LIMIT_HI_BYTE_WR;
		reg_hi = EXT_TEMP_HI_LIMIT_HI_BYTE_WR;
	} else
		return -EINVAL;
	...
	if (*current_lo_limit != lo_limit) {
		...
		err = nct1008_write_reg(data->client, reg_lo, value);
		if (err)
			return err;
	...
	if (*current_hi_limit != hi_limit) {
		...
		err = nct1008_write_reg(data->client, reg_hi, value);
		if (err)
			return err;
...
static void nct1008_update(struct thermal_zone_device *thz,
			   struct nct1008_data *data)
{
	...
	struct thermal_trip_info *trip_state, *trips;
	long temp, trip_temp, hysteresis_temp;
	int count;
	enum events type = 0;

	if (thz->ops == &nct_int_ops)
		trips = data->plat_data.local_trips;
	else if (thz->ops == &nct_ext_ops)
		trips = data->plat_data.trips;
	else
		return;
	...
	nct1008_thermal_set_limits(thz, data, low_temp, high_temp);
}
					...
					unsigned long *temp)
{
	...
	ret = nct1008_get_temp(&client->dev, &temp_ext_milli, NULL);
	if (ret < 0)
		return ret;
				...
				unsigned long *temp)
{
	...
	ret = nct1008_get_temp(&client->dev, NULL, &temp_local_milli);
	if (ret < 0)
		return ret;
			...
			struct thermal_cooling_device *cdev)
{
	...
	if (thz->ops == &nct_int_ops) {
		for (i = 0; i < thz->trips; i++) {
			if (!strcmp(data->plat_data.local_trips[i].cdev_type,
				    cdev->type)) {
				thermal_zone_bind_cooling_device(thz, i, cdev,
					data->plat_data.local_trips[i].upper,
					data->plat_data.local_trips[i].lower);
				bind = true;
			}
		}
	} else if (thz->ops == &nct_ext_ops) {
			  ...
			  struct thermal_cooling_device *cdev)
{
	...
	if (thz->ops == &nct_int_ops) {
		for (i = 0; i < thz->trips; i++) {
			if (!strcmp(data->plat_data.local_trips[i].cdev_type,
				    cdev->type))
				thermal_zone_unbind_cooling_device(thz, i,
								   cdev);
		}
	} else if (thz->ops == &nct_ext_ops) {
		for (i = 0; i < thz->trips; i++) {
...
static int nct1008_get_trip_temp(struct thermal_zone_device *thz,
				 int trip,
				 unsigned long *temp)
{
	struct nct1008_data *data = thz->devdata;
	...
	if (thz->ops == &nct_int_ops)
		trip_state = &data->plat_data.local_trips[trip];
	else if (thz->ops == &nct_ext_ops)
...
static int nct1008_set_trip_temp(struct thermal_zone_device *thz,
				 int trip,
				 unsigned long temp)
{
	struct nct1008_data *data = thz->devdata;
	...
	if (thz->ops == &nct_int_ops) {
		data->plat_data.local_trips[trip].trip_temp = temp;
		nct1008_update(data->nct_int, data);
	} else if (thz->ops == &nct_ext_ops) {
		data->plat_data.trips[trip].trip_temp = temp;
		nct1008_update(data->nct_ext, data);
	} else
		return -EINVAL;
...
static int nct1008_get_trip_type(struct thermal_zone_device *thz,
				     int trip,
				     enum thermal_trip_type *type)
{
	...
	if (thz->ops == &nct_int_ops)
		*type = data->plat_data.local_trips[trip].trip_type;
	else if (thz->ops == &nct_ext_ops)
...
static struct thermal_zone_device_ops nct_ext_ops = {
	...
	.get_trip_type = nct1008_get_trip_type,
	.get_trip_temp = nct1008_get_trip_temp,
	.set_trip_temp = nct1008_set_trip_temp,
				...
				const struct i2c_device_id *id)
{
	...
	mask = 0;
	for (i = 0; i < data->plat_data.num_local_trips; i++)
		mask |= (1 << i);

	data->nct_int = thermal_zone_device_register(nct_int_name,
					data->plat_data.num_local_trips,
					mask,
					data,
					&nct_int_ops,
					data->plat_data.local_tzp,
...
static int nct1008_suspend_wakeup(struct device *dev)
{
	...
	if (ext_temp > data->plat_data.suspend_ext_limit_lo)
		err = nct1008_thermal_set_limits(data->nct_ext, data,
