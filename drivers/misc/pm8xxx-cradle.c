...
static int __devinit pm8xxx_cradle_probe(struct platform_device *pdev)
{
	...
	if (cradle->pdata->hallic_pouch_detect_pin > 0) {
		...
		ret = request_irq(hall_pouch_gpio_irq, pm8xxx_pouch_irq_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
				HALL_IC_DEV_NAME, cradle);
		if (ret > 0) {
	...
	printk("%s : irq_flags = [%d]\n", __func__, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
