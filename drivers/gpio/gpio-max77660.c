		...
		struct max77660_platform_data *pdata)
{
	...
	ret = max77660_reg_read(parent, MAX77660_PWR_SLAVE,
				MAX77660_REG_GPIO_IRQ1, &val);
	if (ret < 0) {
		dev_err(max77660_gpio->dev, "GPIO_IRQ1 read failed: %d\n", ret);
		return ret;
	}

	ret = max77660_reg_read(parent, MAX77660_PWR_SLAVE,
				MAX77660_REG_GPIO_IRQ2, &val);
	if (ret < 0) {
		dev_err(max77660_gpio->dev, "GPIO_IRQ2 read failed: %d\n", ret);
		return ret;
	}
