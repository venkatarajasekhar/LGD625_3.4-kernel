...
#if defined(CONFIG_RTC_INTF_SECCLK)
    dev_dbg(rtc->dev, "adjust secclk in max77660_rtc_set_time\n");
    secclk_rtc_changed(max77660_rtc_read_time, dev, tm);
#endif
 ...
 */
	...
	ret = max77660_reg_clr_bits(rtc->parent, MAX77660_RTC_SLAVE,
				MAX77660_RTC_SMPL, BIT(7));
	if (ret < 0) {
		pr_info("[POWER] SMPL_EN CLEAR FAILED!!!!! \n");
		return ret;
	}
	ret = max77660_reg_write(rtc->parent, MAX77660_RTC_SLAVE,
             MAX77660_RTC_SMPL,0x80);
	if (ret <0){
		pr_info("[POWER] setting SMPL 0.5s FAILED!!! \n");
        return ret;
	}else{
        pr_info("[POWER] setting SMPL 0.5s SUCCESS !!! \n");
	}

    ret = max77660_reg_clr_bits(rtc->parent, MAX77660_RTC_SLAVE,
			MAX77660_RTC_IRQ_MASK, MAX77660_RTC_IRQ_ALARM1_MASK);
    if (ret < 0) {
		pr_info("[POWER] SMPL MASK CLEAR FAILED!!!!! \n");
		return ret;
	}else{
        pr_info("[POWER] SMPL MASK CLEAR SUCCESSED!!!!!!!! \n");
	}
