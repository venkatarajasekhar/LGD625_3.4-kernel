...
#ifdef CONFIG_MACH_PDA
int max77660_lge_get_adc(int pin_name) {

	int ret;
	struct max77660_adc *adc = iio_priv(lge_iodev);

	mutex_lock(&lge_iodev->mlock);
	ret = max77660_adc_read_channel(adc, pin_name);
	if (ret < 0) {
		dev_err(adc->dev, "ADC start coversion failed\n");
		mutex_unlock(&lge_iodev->mlock);
		return ret;
	}
	mutex_unlock(&lge_iodev->mlock);
	printk("%s: adc: %d \n", __func__, ret);
