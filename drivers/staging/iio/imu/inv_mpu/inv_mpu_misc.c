	...
	u8 *data, int size)
{
	...
	int result;
	u16 memaddr;
	u8 firmware[MPU_MEM_BANK_SIZE] = {0, };   /*                                  */
...
static int inv_accel_dmp_cal(struct inv_mpu_state *st)
{
	...
	int result;
	u8 regs[3];
	const u8 tmp[4] = { DINA0C, DINAC9, DINA2C,0x0}; /*                                                                                 */
	inv_accel_orient =
		inv_orientation_matrix_to_scaler(st->plat_data.orientation);
...
/*                                    */
/*
  Buffer overflow, array index of 'input_accel_dmp_bias' may be out of bounds.
  Array 'input_accel_dmp_bias' of size 3 may use index value(s) 3
*/
#if 0 /* Original */
	for (i = 0; i < 3; i++)
		accel_bias_body[i] =
