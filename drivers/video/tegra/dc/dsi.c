	...
	if (dsi->enabled) {
		...
		if (dsi->info.panel_reset) {
			...
			if (lge_get_board_revno() >= HW_REV_PDA_C) {
				err = tegra_dsi_send_panel_cmd(dc, dsi,
					dsi->info.dsi_init_cmd2,
					dsi->info.n_init_cmd2);
				if (err < 0) {
					dev_err(&dc->ndev->dev,
					"dsi: error while sending dsi init cmd2\n");
					goto fail;
				}
			}
#endif
		 ...
		 */
		if (dsi->info.panel_send_dc_frames)
			tegra_dsi_send_dc_frames(dc, dsi, 1); //                      
...
#if defined(CONFIG_MACH_PDA)
	struct tegra_dsi_cmd *p_init_cmd2 = NULL;
#endif
...
/*                                                                                            
                                                                                             ...
                                                                                             */
	if(lge_get_board_revno() >= HW_REV_PDA_C){
		p_init_cmd2 = kzalloc(sizeof(*p_init_cmd2) *
					p_dsi->n_init_cmd2, GFP_KERNEL);
		if (!p_init_cmd2)
			return -ENOMEM;
	}
#endif
...
/*                                                                                            
                                                                                             ...
                                                                                             */
	if(lge_get_board_revno() >= HW_REV_PDA_C){
		/* Copy panel init cmd2 */
		if (p_dsi->dsi_init_cmd2) {
			err = tegra_dc_dsi_cp_p_cmd(p_dsi->dsi_init_cmd2,
								p_init_cmd2, p_dsi->n_init_cmd2);
			if (err < 0)
				goto err_free;
			dsi->info.dsi_init_cmd2 = p_init_cmd2;
		}
	}
#endif
...
/*                                                                                            
                                                                                             ...
                                                                                             */
	if(lge_get_board_revno() >= HW_REV_PDA_C){
		for (i = 0; i < dsi->info.n_init_cmd2; i++) {
			if (dsi->info.dsi_init_cmd2[i].pdata)
				kfree(dsi->info.dsi_init_cmd2[i].pdata);
		}
		kfree(dsi->info.dsi_init_cmd2);
	}
#endif
	/* Disable dc stream */
	if (dsi->status.dc_stream == DSI_DC_STREAM_ENABLE)
