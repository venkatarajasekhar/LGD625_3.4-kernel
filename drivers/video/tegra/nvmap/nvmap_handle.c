...
void _nvmap_handle_free(struct nvmap_handle *h)
{
	...
	if (h->pgalloc.area) {
		trace_printk("free_iova: (%s) h=%p, start=0x%08x, end=0x%08x\n",
			current->comm, h,
			h->pgalloc.area->iovm_start,
			h->pgalloc.area->iovm_start + h->size);
	} else {
		trace_printk("free_iova: (%s) h=%p, start=0x%08x, end=0x%08x (no area pt)\n",
			current->comm, h, h->pgalloc.iovm_addr, h->pgalloc.iovm_addr + h->size);
	}
