...
skip_pup_dn:
	...
	// Based on GPIOs Prograrmming Matrix
	// Set GPIO8 to Input High-Z

	dev_info(&pdev->dev, "[PIN Ctrl] %s ", "start GPIO8");
	//PUE disabled
        ret = max77660_reg_clr_bits(parent, MAX77660_PWR_SLAVE,
		MAX77660_REG_PUE2_GPIO, BIT(0));

	if (ret < 0) {
		dev_err(&pdev->dev,
			"PUE2 update failed: %d\n", ret);
		return ret;
	}
	//PDE disabled
	ret = max77660_reg_clr_bits(parent, MAX77660_PWR_SLAVE,
		MAX77660_REG_PDE2_GPIO, BIT(0));

	if (ret < 0) {
		dev_err(&pdev->dev,
			"PDE2 update failed: %d\n", ret);
		return ret;
	}
	// DIR Input
	ret = max77660_reg_set_bits(parent, MAX77660_PWR_SLAVE,
		MAX77660_REG_CNFG_GPIO0+8, BIT(1));

	if (ret < 0) {
		dev_err(&pdev->dev,
			"CNFG_GPIO8 update failed: %d\n", ret);
		return ret;
	}
