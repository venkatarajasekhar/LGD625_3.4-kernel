...
static char *const tegra_udc_extcon_cable[] = {
	...
	[CONNECT_TYPE_LGE_FACTORY] = "Lge-factory-source", /* juya.kim */
	NULL,
};
		...
		if (udc->connect_type == CONNECT_TYPE_SDP) {
			if (lge_get_factory_boot()) {
				schedule_delayed_work(&udc->non_std_charger_work,
					msecs_to_jiffies(FACTORY_CHARGER_DET_TIME_MS));
			} else {
				schedule_delayed_work(&udc->non_std_charger_work,
...
static int tegra_usb_set_charging_current(struct tegra_udc *udc)
{
	...
	/* check if this func is called!!!
	  * if The device is boot with factory cable and never unpluged, This fun. definately runs.
	  * However, if the cable is disconnected as kernel boots, this fun. never called so, is_factory_cable() knows
	  * if this func. needs to return true or not.
	  */
	if(get_fcable_disconnect_check_flag()==1)
		clear_fcable_disconnect_check_flag();
	/*
         * factory cable & udc->connect_type == CONNECT_TYPE_NONE
         */
	if((lge_get_boot_mode() == LGE_BOOT_MODE_PIFBOOT2
		|| lge_get_boot_mode() == LGE_BOOT_MODE_QEM_56K
		|| lge_get_boot_mode() == LGE_BOOT_MODE_QEM_130K
		|| lge_get_boot_mode() == LGE_BOOT_MODE_MINIOS)
		&& udc->connect_type != CONNECT_TYPE_NONE
		&& get_firstboot_factory_flag() == 1) {

		udc->connect_type = CONNECT_TYPE_LGE_FACTORY;
		pr_info("[Factory_Cable] Enter the first case. \n");
	}
	...
	case CONNECT_TYPE_NONE:
		...
		pr_info("[Factory_Cable] Flag is cleared.!!! \n");
		clear_firstboot_factory_flag();
		max_ua = 0;
		break;
...
#ifdef CONFIG_MACH_PDA
		max_ua = USB_CHARGING_LGE_INBOX_TA_CURRENT_LIMIT_UA;
#else
	...
	case CONNECT_TYPE_NON_STANDARD_CHARGER:
		...
		break;
	case CONNECT_TYPE_LGE_FACTORY:
		pr_info("[Factory_Cable] USB_CHARGING_LGE_FACTORY_UA is set \n");
		max_ua = USB_CHARGING_LGE_FACTORY_UA;
		break;
	default:
...
static int tegra_detect_cable_type(struct tegra_udc *udc)
{
	...
	pr_info("USB : %s %d", __func__, udc->connect_type);
	/*
...
static int tegra_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	pr_info("tegra-otg: %s(%d) turn VBUS state from %s to %s", __func__, __LINE__,
...
{
#if defined(CONFIG_USB_G_LGE_ANDROID) && defined(CONFIG_USB_G_LGE_FULLSPEED)
	u32 temp, portsc;
#else
	u32 temp;
#endif
  ...
  */
	if (LGE_BOOT_MODE_QEM_130K == lge_get_boot_mode())
	{
		/* read the portsc register */
		portsc = udc_readl(udc, USB_HOSTPCX_DEVLC_REG_OFFSET);
		/* set the usb full-speed */
		portsc |= PORTSCX_PORT_FORCE_FULL_SPEED;
		udc_writel(udc, portsc, USB_HOSTPCX_DEVLC_REG_OFFSET);
	}
#endif
