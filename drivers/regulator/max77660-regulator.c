...
/* Maximum delay from the I2C final rising edge to the buck starting to
 * ramp the output. */
#define BUCK_RAMP_START_TIME_USEC	1
/* The minimum ramp rate time is this percentage of the nominal. */
#define BUCK_RAMP_RATE_MIN_RATIO	90
/* Maximum filter Time. */
#define BUCK_FILTER_MAX_TIME_USEC	12
					     ...
					     int min_uV, int max_uV)
{
	...
	if (steps == 1) {
		val = DIV_ROUND_UP(min_uV - rinfo->min_uV, rinfo->step_uV);
	...
	if (reg->ramp_delay > 0) {
		int ramp_delay = DIV_ROUND_UP(
			reg->ramp_delay * BUCK_RAMP_RATE_MIN_RATIO, 100);
		int delay_us = BUCK_RAMP_START_TIME_USEC
			+ DIV_ROUND_UP(change_uV, ramp_delay);
		delay_us = max(BUCK_FILTER_MAX_TIME_USEC, delay_us);
		if (delay_us > 1000) {
...
static int max77660_regulator_enable(struct regulator_dev *rdev)
{
	...
	/* ES 1.1 and 1.2 suggest to keep LDO1/LDO14 in GLPM */
				       ...
				       unsigned int mode)
{
	...
	if (reg->rinfo->id == MAX77660_REGULATOR_ID_BUCK6 ||
		...
		/* buck6/7 can be auto mode from ES1.2 */
		if (!(max77660_is_es_1_2(reg->dev)) && (mode == REGULATOR_MODE_FAST))
	...
	*/
	...
	if (rinfo->type == REGULATOR_TYPE_BUCK) {
		...
		if ((reg->rinfo->id >= MAX77660_REGULATOR_ID_BUCK1) &&
			...
			(reg->rinfo->id <= MAX77660_REGULATOR_ID_BUCK7)) {
			mask |= MAX77660_BUCK6_7_CNFG_FPWM_MASK;
			/* ES 1.2 suggest to remove all BUCKS
			 * from FPWM mode.
			 */
			if ((pdata->flags & SD_FORCED_PWM_MODE) &&
...
static int max77660_regulator_probe(struct platform_device *pdev)
{
	...
	for (id = 0; id < MAX77660_REGULATOR_ID_NR; ++id) {
		...
		if (reg->ramp_delay < 0)
			dev_info(&pdev->dev,
				"Ramp Delay of rail %s is  %d uV/us\n",
