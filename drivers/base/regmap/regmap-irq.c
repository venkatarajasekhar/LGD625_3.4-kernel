...
static char *max77660_global_irq[] = {
	"max77660_tjalarm2",
	"max77660_tjalarm1",
	"max77660_syslow",
	"max77660_i2cwdt",
...
extern bool is_max77660_pmic_wakeup;
...
static irqreturn_t regmap_irq_thread(int irq, void *d)
{
	...
	for (i = 0; i < chip->num_irqs; i++) {
		if (data->status_buf[chip->irqs[i].reg_offset /
			...
			if (is_max77660_pmic_wakeup) {
				if (irq == 118) {
					/*                                     */
					if (i < ARRAY_SIZE(max77660_top_irq))
						pr_err("%s caused wakeup\n",
							max77660_top_irq[i]);
				} else if (irq == 711) {
					/*                                     */
					if (i < ARRAY_SIZE(max77660_global_irq))
						pr_err("%s caused wakeup\n",
							max77660_global_irq[i]);
				}
			}
