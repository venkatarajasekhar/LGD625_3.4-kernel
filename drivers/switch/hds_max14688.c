...
static int max14688_log_level = 0;
...
static void max14688_lookup_button (struct max14688 *me, int micZ, int leftZ)
{
	int i;

	for (i = 0; i < me->num_of_button_matches; i++) {
		if (unlikely(__match_key(&me->button_matches[i].mic,  micZ ) &&
					__match_key(&me->button_matches[i].left, leftZ))) {
			me->matched_button = i;
			return;
		}
	}
...
static void max14688_det_work (struct work_struct *work)
{
	...
	/* Check INT bit for avoiding JIG mode */
	if (unlikely(max14688_get_status(me, STATUS_INT))) {
		log_info("JIG power detected\n");
		goto jig_power_detected;
	}
...
static void max14688_irq_button_pressed (struct max14688 *me)
{
	...
	max14688_lookup_button(me, micZ, leftZ);

	if (unlikely(!__present_valid_button(me))) {
		log_warn("unknown button - mic %d, left %d\n", micZ, leftZ);
		goto out;
	}
		...
		struct device_attribute *devattr, char *buf)
{
	...
	rc += (int)snprintf(buf+rc, PAGE_SIZE,
			"Detected Jack           %s (%d)\n",
			__present_valid_jack(me)? __current_jack_name(me) : "(null)",
			me->matched_jack);
	rc += (int)snprintf(buf+rc, PAGE_SIZE,
			"Detected Button         %s (%d)\n",
			__present_valid_button(me)? __current_button_name(me) : "(null)",
			me->matched_button);
...
static int max14688_read_mic_impedence (struct device *dev)
{
	...
	rc = (max14688_get_status(me, STATUS_SWD) == 0);
...
{
	struct max14688_platform_data* pdata;
	struct max14688 *me;
	u8 chip_id, chip_rev;
	int i, rc;
	u8 pincontrol2 = 0;
	...
	if (unlikely(me->irq < 0)) {
		...
		rc = request_threaded_irq(me->irq, NULL, max14688_isr,
				IRQF_ONESHOT | pdata->irq_trigger, DRIVER_NAME, me);
		if (unlikely(rc < 0)) {
