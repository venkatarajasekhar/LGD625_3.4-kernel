...
#define DISABLE_MIC_BIAS	/* FIXME: for PD_A */
...
static int lge_hsd_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct hsd_info *hi = (struct hsd_info *)platform_get_drvdata(pdev);

	gpio_set_value(hi->gpio_mode, 0);
	return 0;
}

static int lge_hsd_resume(struct platform_device *pdev)
{
	struct hsd_info *hi = (struct hsd_info *)platform_get_drvdata(pdev);

	gpio_set_value(hi->gpio_mode, 1);
	return 0;
}
#else
#define lge_hsd_suspend		NULL
#define lge_hsd_resume		NULL
#endif
