...
extern bool max17048_is_chg_done(void);
#endif
...
static void max77660_power_off(void)
{
	...
	if (!chip)
		return;
	printk("[POWER] max77660_power_off!!! \n");
...
static void max77660_power_reset(void)
{
	...
	if (!chip)
		return;
	printk("[POWER] max77660_power_reset!!! \n");
...
#ifdef CONFIG_MACH_PDA//AT%CHCOMP, AT%PMRST
               ...
               struct device_attribute *attr, char *buf)
{
	...
	max77660_reg_read(chip->dev, MAX77660_PWR_SLAVE,
			MAX77660_CHARGER_DETAILS2,&status);

	new_status = status << 4;
	is_chg_done = max17048_is_chg_done();
	pr_info("%s: CHG Done : %d mA\n", __func__, is_chg_done);

	if(new_status==0x50 || is_chg_done==true){
		r = snprintf(buf, 3, "%d\n", 1);
		dev_info(chip->dev,"charging complete - buf : %s\n",buf);
	}
	else{
		r = snprintf(buf, 3, "%d\n", 0);
		dev_info(chip->dev,"under charging - buf : %s\n",buf);
	}

	return r;
}

DEVICE_ATTR(at_pmrst, 0640, max77660_power_reset, NULL);
DEVICE_ATTR(at_chcomp, 0644, at_chg_complete_show, NULL);
#endif
			  ...
			  const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(&client->dev,"AT_PMRST File device creation failed: %d\n", ret);
		goto out_exit;
	}

	ret = device_create_file(&client->dev, &dev_attr_at_chcomp);
	if (ret < 0) {
		dev_err(&client->dev,"AT_CHCOMP File device creation failed: %d\n", ret);
		goto out_exit;
	}
#endif
	return 0;
