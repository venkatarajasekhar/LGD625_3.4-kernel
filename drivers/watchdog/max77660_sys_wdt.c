...
static struct max77660_sys_wdt *g_wdt;
...
int max77660_reset_sys_wdt_ex(const char *from)
{
	return max77660_reset_sys_wdt(g_wdt, from);
}
EXPORT_SYMBOL(max77660_reset_sys_wdt_ex);
#endif
...
static int max77660_sys_wdt_start(struct watchdog_device *wdt_dev)
{
	...
	int ret;
	printk("[POWER-WTD] max77660_sys_wdt_start!!! \n");
...
static int max77660_sys_wdt_stop(struct watchdog_device *wdt_dev)
{
	struct max77660_sys_wdt *wdt = watchdog_get_drvdata(wdt_dev);
	int ret;
	printk("[POWER-WTD] max77660_sys_wdt_stop !!! \n");
...
static int __devinit max77660_sys_wdt_probe(struct platform_device *pdev)
{
	...
	if (pdata && (pdata->system_watchdog_timeout > 0)) {
		...
		printk("[POWER-WDT] before max77660_sys_wdt_start!!! \n");
		...
		if (wdt->sw_wdt_reset && !pdata->kick_by_tegraWDT) {
#else
		if (wdt->sw_wdt_reset) {
#endif
...
static int max77660_sys_wdt_suspend(struct device *dev)
{
	...
	int ret;
	printk("[POWER-WDT] max77660_sys_wdt_suspend 1\n");
	if (wdt->timeout > 0) {
	printk("[POWER-WDT] max77660_sys_wdt_suspend 2\n");
		mutex_lock(&wdt->reset_lock);
		wdt->suspended = true;
		max77660_reset_sys_wdt(wdt, "suspend");
		mutex_unlock(&wdt->reset_lock);
	}
	printk("[POWER-WDT] max77660_sys_wdt_suspend 3\n");
	if (device_may_wakeup(dev)) {
		printk("[POWER-WDT] max77660_sys_wdt_suspend 4\n");
		enable_irq_wake(wdt->irq);
	} else if (wdt->timeout > 0) {
	printk("[POWER-WDT] max77660_sys_wdt_suspend 5\n");
...
static int max77660_sys_wdt_resume(struct device *dev)
{
	...
	printk("[POWER-WDT] max77660_sys_wdt_resume 1\n");
	if (device_may_wakeup(dev)) {
		printk("[POWER-WDT] max77660_sys_wdt_resume 2\n");
		disable_irq_wake(wdt->irq);
	} else if (wdt->timeout > 0) {
		printk("[POWER-WDT] max77660_sys_wdt_resume 3\n");
		printk("[POWER-WDT] before max77660_sys_wdt_start in max77660_sys_wdt_resume!!!\n");
		ret = max77660_sys_wdt_start(&wdt->wdt_dev);
		if (ret < 0)
			dev_err(wdt->dev, "wdt start failed: %d\n", ret);
	}
	if (wdt->timeout > 0) {
		printk("[POWER-WDT] max77660_sys_wdt_resume 4\n");
		mutex_lock(&wdt->reset_lock);
		wdt->suspended = false;
