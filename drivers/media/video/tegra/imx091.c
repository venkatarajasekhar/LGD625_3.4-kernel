...
#define IMX091_ANALOG_GAIN_MAX  16000 /* / _INT2FLOAT_DIVISIOR */
#define IMX091_DIGITAL_GAIN_MAX 3000 /* / _INT2FLOAT_DIVISIOR */
#define IMX091_DIGITAL_GAIN_MIN 1000 /* / _INT2FLOAT_DIVISIO */
#define IMX091_MAX_GAIN (IMX091_ANALOG_GAIN_MAX * IMX091_DIGITAL_GAIN_MAX / 1000)
...
struct imx091_info {
	...
	struct imx091_eeprom_data eeprom[IMX091_EEPROM_NUM_BLOCKS];
	u8 eeprom_buf[IMX091_EEPROM_SIZE];
#endif
};
...
static struct nvc_imager_cap imx091_dflt_cap = {
	...
	.pixel_types[0]		= 0x101,    /* 0x100 : RGGB, 0x101 : BGGR */
	.orientation		= 0,
	.direction		= 0,
...
static struct imx091_reg imx091_4160x3120_i2c[] = {
	/* Reset */
	{0x0100, 0x00},
	{0x0103, 0x01},
...
static struct imx091_reg imx091_4160x2340_i2c[] = {
	/* Reset */
	{0x0100, 0x00},
	{0x0103, 0x01},
	{IMX091_TABLE_WAIT_MS, IMX091_WAIT_MS},
...
static struct imx091_reg imx091_2080x1560_i2c[] = {
	/* Software reset */
	{0x0100, 0x00},
	{0x0103, 0x01},
	{IMX091_TABLE_WAIT_MS, IMX091_WAIT_MS},
...
static struct imx091_reg imx091_2080x1170_i2c[] = {
	/* Reset */
	{0x0100, 0x00},
	{0x0103, 0x01},
	{IMX091_TABLE_WAIT_MS, IMX091_WAIT_MS},
 ...
 */
static struct imx091_mode_data imx091_4160x3120 = {
	.sensor_mode = {
		.res_x			= 4160,
		.res_y			= 3120,
		.active_start_x		= 0,
		.active_stary_y		= 0,
		.peak_frame_rate	= 15200, /* / _INT2FLOAT_DIVISOR */
		.pixel_aspect_ratio	= 1000, /* / _INT2FLOAT_DIVISOR */
		.pll_multiplier		= 23500, /* / _INT2FLOAT_DIVISOR */
	...
	.sensor_dnvc = {
		...
		.line_length		= 0x120C,
		.frame_length		= 0x0C8C,
		.min_frame_length	= 0x0C8C,
		.max_frame_length	= 0xFFFF,
		.min_gain		= 1000, /* / _INT2FLOAT_DIVISOR */
		.max_gain		= IMX091_MAX_GAIN, /* / _INT2FLOAT_DIVISOR */
		.analog_max_gain	= IMX091_ANALOG_GAIN_MAX,
		.digital_min_gain	= 1000,
		.digital_max_gain	= IMX091_DIGITAL_GAIN_MAX,
	...
	.p_mode_i2c			= imx091_4160x3120_i2c,
};

static struct imx091_mode_data imx091_4160x2340 = {
	.sensor_mode = {
		.res_x			= 4160,
		.res_y			= 2340,
		.active_start_x		= 0,
	...
	.p_mode_i2c			= imx091_4160x2340_i2c,
};

static struct imx091_mode_data imx091_2080x1560 = {
	.sensor_mode = {
		.res_x			= 2080,
		.res_y			= 1560,
		.active_start_x		= 0,
		.active_stary_y		= 0,
		.peak_frame_rate	= 30410, /* / _INT2FLOAT_DIVISOR */
		.pixel_aspect_ratio	= 1000, /* / _INT2FLOAT_DIVISOR */
	...
	.p_mode_i2c			= imx091_2080x1560_i2c,
};

static struct imx091_mode_data imx091_2080x1170 = {
	.sensor_mode = {
		.res_x			= 2080,
		.res_y			= 1170,
		.active_start_x		= 0,
	...
	.p_mode_i2c			= imx091_2080x1170_i2c,
};
...
static void imx091_edp_register(struct imx091_info *info)
{
	...
	/* WAR: Disable IMX091 System EDP for until EDP tuning is done. */
	return;
...
static inline void imx091_gain_reg(struct imx091_reg *regs, u32 gain, u32 dgain)
{
	regs->addr = 0x0205;
	regs->val = gain & 0xFF;
...
{
	struct imx091_reg reg_list[16];
	int err;
	unsigned int dgain = 0;

	dgain = (mode->dgain_upper << 8 | mode->dgain_lower);
	pr_info("%s: dgain = %u, (upper = %d, lower = %d)\n",
			__func__, dgain,
			mode->dgain_upper,
			mode->dgain_lower);
	...
	imx091_gain_reg(reg_list + 5, mode->gain, dgain);
	if (dgain >= 0x100) {
		reg_list[14].addr = 0x0104;
		reg_list[14].val = 0x00;
		reg_list[15].addr = IMX091_TABLE_END;
	}
	else {
				...
				struct nvc_imager_ae *ae)
{
	...
	if (ae->gain_enable) {
		if (ae->dgain >= 0x100) {
			imx091_gain_reg(reg_list + count, ae->gain, ae->dgain);
			count += 9;
		}
		else {
			imx091_gain_reg(reg_list + count, ae->gain, 0);
			count += 1;
		}
		pr_debug("%s: gain = %d, dgain = %d\n", __func__,
				ae->gain, ae->dgain);
	}
...
#if USE_REGMAP
static int
imx091_eeprom_device_release(struct imx091_info *info)
{
	int i;

	for (i = 0; i < IMX091_EEPROM_NUM_BLOCKS; i++) {
...
#if USE_REGMAP
static int
imx091_eeprom_device_init(struct imx091_info *info)
{
	char *dev_name = "eeprom_imx091";
	...
	for (i = 0; i < IMX091_EEPROM_NUM_BLOCKS; i++) {
		...
		info->eeprom[i].brd.addr = IMX091_EEPROM_ADDRESS + i;
		...
		if (IS_ERR(info->eeprom[i].regmap)) {
			...
			imx091_eeprom_device_release(info);
			return err;
		}
	}
...
#if USE_REGMAP
static int
imx091_read_eeprom(struct imx091_info *info, u8 reg, u16 length, u8 *buf)
{
	return regmap_raw_read(info->eeprom[0].regmap, reg, &buf[reg], length);
}

static int
imx091_write_eeprom(struct imx091_info *info, u16 addr, u8 val)
{
...
/* if needed, change below to match module */

static int
imx091_read_eeprom(struct i2c_client *client, u8 reg, u16 length, u8 *buf)
{
int ret;
struct i2c_msg msg;
if (!client->adapter)
...
//                                                                                  
		is_flash_on = mode.gain ? true : false;
//                                               
	...
	{
		imx091_read_eeprom(info->i2c_client, 0, EEPROM_SIZE, eeprom);

		if (copy_to_user((void __user *)arg, eeprom, EEPROM_SIZE)) {
			dev_err(&info->i2c_client->dev,
	...
	case NVC_IOCTL_SET_EEPROM_DATA:
	{
		...
		if (copy_from_user(info->eeprom_buf,
			(const void __user *)arg, IMX091_EEPROM_SIZE)) {
			dev_err(&info->i2c_client->dev,
				"%s:Failed to read from user buffer\n",
				__func__);
			return -EFAULT;
		}
		for (i = 0; i < IMX091_EEPROM_SIZE; i++) {
			imx091_write_eeprom(info,
				i,
				info->eeprom_buf[i]);
			msleep(20);
		}
...
static int imx091_open(struct inode *inode, struct file *file)
{
	...
	err = imx091_pm_api_wr(info, NVC_PWR_ON);
	dev_dbg(&info->i2c_client->dev, "%s -----\n", __func__);
	return 0;
}
...
#if USE_REGMAP
	/* eeprom interface */
	err = imx091_eeprom_device_init(info);
	if (err) {
		dev_err(&client->dev,
...
#endif
	dev_dbg(&client->dev, "%s -----\n", __func__);
	pr_err("[imx091]: probing sensor done.\n");
	return 0;
}
