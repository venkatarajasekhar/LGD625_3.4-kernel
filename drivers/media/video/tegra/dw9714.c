...
#define DW9714_FOCAL_LENGTH		(3.97f)
#define DW9714_FNUMBER			(2.4f)
...
static int dw9714_i2c_wr16(struct dw9714_info *info, u16 val)
{
	...
	do {
		...
		dev_err(&info->i2c_client->dev,
			"[CAM] dw9714: i2c transfer failed, " \
			"retrying value: %x\n", val);
		msleep(3);
...
static int dw9714_power_get(struct dw9714_info *info)
{
	...
	dw9714_regulator_get(info, &pw->vdd, "vdd_cam_1v8");
...
static int dw9714_position_wr(struct dw9714_info *info, s32 position)
{
	...
	dev_dbg(&info->i2c_client->dev, "[CAM] DW9714: %s: cur_pos : %d\n", __func__, position);
...
static int dw9714_param_rd(struct dw9714_info *info, unsigned long arg)
{
	...
	case NVC_PARAM_FNUMBER:
		/*                                                              */
		f_number = DW9714_FNUMBER;
		data_ptr = &f_number;
		data_size = sizeof(f_number);
		/*                                                              */
		break;
	case NVC_PARAM_CAPS:
