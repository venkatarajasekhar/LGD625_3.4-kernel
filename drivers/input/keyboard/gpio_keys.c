...
/*                                                             */
		if(button->code == KEY_VOLUMEUP)
			printk("[gpio-key] KEY_VOLUMEUP %s\n",(state) ? " PRESS" : " RELEASE");
		else if(button->code == KEY_VOLUMEDOWN)
			printk("[gpio-key] KEY_VOLUMEDOWN %s\n",(state) ? " PRESS" : " RELEASE");
		else
			printk("[gpio-key] gpio_keys_gpio_report_event button->code = %d, %s\n", button->code, state? " PRESS" : " RELEASE");
/*                                                */
	}
	input_sync(input);
}
...
static void gpio_keys_report_event(struct gpio_button_data *bdata)
{
	...
	if( button->irq == (TEGRA_NR_IRQS + MAX77660_IRQ_GLBL_EN0_F))
	    state = 0;
	else if( button->irq == (TEGRA_NR_IRQS + MAX77660_IRQ_GLBL_EN0_R))
	    state = 1;
	else{
	}
	...
	/*                                                             */
	if(state)
		printk("[gpio-key] power-key : PRESS\n");
	else 
		printk("[gpio-key] power-key : RELEASE\n");
	/*                                                */
	input_event(input, type, button->code, state);
	input_sync(input);
}
