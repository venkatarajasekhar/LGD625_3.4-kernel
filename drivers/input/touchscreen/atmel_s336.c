...
#if defined(CONFIG_TOUCHSCREEN_LGE_BLOCK_MASK)
static int touch_report_mask = 1;
static bool touch_report_log = true;
#endif//
...
int mxt_get_reference_chk(struct mxt_data *data)
{
	...
	int ret = 0;
	int i = 0;
	u8 err_tot_x[MXT_CHANNEL_X_NUM] = {0}, err_tot_y[MXT_CHANNEL_Y_NUM] = {0};
	...
	for (read_page = 0 ; read_page < end_page; read_page++) {
		...
		for (i = 0; i < NODE_PER_PAGE; i++) {
			if (num < MAX_BUF_COUNT)
				buf[num] =( ((u16)ret_buf[i*2+1] << 8) + (u16)ret_buf[i*2] ) - REF_OFFSET_VALUE;
			else {
				TOUCH_INFO_MSG("num is over MAX_BUF_COUNT !!\n");
				break;
			}
			...
			if ((curx >= data->channel_size.start_x + data->channel_size.size_x) || (curx < data->channel_size.start_x) ||
				...
				num++;
				cury++;
				if (cury >= MXT_CHANNEL_Y_NUM) {
					curx++;
					if(curx >= MXT_CHANNEL_X_NUM)
						curx = 0;
					cury = 0;
				}
				continue;
			...
			if(num >= 335) {
				TOUCH_INFO_MSG("num is over 335 !!\n");
				num = 0;
				break;
			}
...
#if defined(CONFIG_TOUCHSCREEN_LGE_BLOCK_MASK)
	if (unlikely(touch_report_mask == 1994)) {
		if(touch_report_log) {
			TOUCH_INFO_MSG("## You get the magic code in touch. block touch event in touch driver!! \n");
			touch_report_log = false;
		}
		return;
	}
#endif
...
#if defined(CONFIG_TOUCHSCREEN_LGE_BLOCK_MASK)
static ssize_t show_touch_report_status(struct mxt_data *data, char *buf)
{
	int ret = 0;

	ret += sprintf(buf+ret,"%s : %d \n", __func__, touch_report_mask);

	return ret;
}


static ssize_t store_touch_report_status(struct mxt_data *data, const char *buf, size_t count)
{
	sscanf(buf, "%d", &touch_report_mask);
...
#if defined(CONFIG_TOUCHSCREEN_LGE_BLOCK_MASK)
static LGE_TOUCH_ATTR(touch_report_status, S_IRUGO | S_IWUSR, show_touch_report_status, store_touch_report_status);
#endif//
...
#if defined(CONFIG_TOUCHSCREEN_LGE_BLOCK_MASK)
	&lge_touch_attr_touch_report_status.attr,
#endif//
	NULL
};
...
/*                                                                        */
		if (!(data->info)) {
			TOUCH_INFO_MSG("In mxt_read_id_info data->info kzalloc fail \n");
			goto out;
		}
/*                           */
...
/*                                                                 */
		if (!object) {
			TOUCH_INFO_MSG("Invalid object type about MXT_SPT_DYNAMICCONFIGURATIONCONTAINER_T71\n");
			return -EINVAL;
		}
/*                                                */
...
static int __devinit mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	else /*if not using dtsi data, init pdata by board config*/
	{
		data->pdata = client->dev.platform_data;
		data->mxt_knock_on_enable  = data->pdata->knock_on_type;
		TOUCH_INFO_MSG(" Do not use dtsi, and mxt_knock_on_enable %d\n", data->mxt_knock_on_enable);
	}
#if defined(CONFIG_TOUCHSCREEN_LGE_LPWG)
