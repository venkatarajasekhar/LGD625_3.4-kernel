		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (chip->msec_tmout) {
		dev_dbg(chip->dev, "%s: cancel vibrator off tmout wrk\n",
			__func__);
		printk(KERN_INFO "[vibrator] cancel vibrator off tmout wrk\n");
	...
	if(!chip->level) printk(KERN_INFO "[vibrator] vibrator off by instruction\n");
	else printk(KERN_INFO "[vibrator] vibrator on by instruction\n");
	schedule_work(&chip->work);
	mutex_unlock(&chip->enable_lock);
...
static void vibrator_off_cb(struct work_struct *work)
{
	...
	printk(KERN_INFO "[vibrator] vibrator off timeout forced\n");
	mutex_lock(&chip->enable_lock);
...
register_input:
	...
	if (lge_get_board_revno() >= HW_REV_D625_1_0) {
		chip->ldo_gpio = haptic_pdata->ldo_gpio;
		rc = gpio_request(chip->ldo_gpio, "motor_ldo");
		if (rc) {
			printk(KERN_ERR "%s: motor_ldo %d request failed\n", __func__, chip->ldo_gpio);
			return rc;
		}

		rc = gpio_direction_output(chip->ldo_gpio, 1);
		if (rc) {
			printk(KERN_ERR "%s: gpio_timm_config(%#x)=fg%d\n", __func__, chip->ldo_gpio, rc);
			return rc;
		}

		gpio_set_value(chip->ldo_gpio, 1);
	}
	return 0;
...
static void max77660_haptic_shutdown(struct platform_device *pdev)
{
	struct max77660_haptic *chip = platform_get_drvdata(pdev);

	printk(KERN_INFO "[vibrator] shutdown\n");
//	max77660_haptic_enable(chip, false);
	max77660_haptic_configure(chip, false);
	if (chip->mode == MAX77660_EXTERNAL_MODE)
		pwm_disable(chip->pwm);
...
static struct platform_driver max77660_haptic_driver = {
	...
	.shutdown	= max77660_haptic_shutdown,
