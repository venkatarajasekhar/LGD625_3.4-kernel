...
#define APDS9130_INT		90

#define APDS9130_PS_PULSE_NUMBER		16
...
static void apds9130_change_ps_threshold(struct i2c_client *client)
{
  ...
  */
	apds9130_set_pers(client, APDS9130_PPERS_3);
...
static ssize_t apds9130_show_ps_detection(struct device *dev,
            struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct apds9130_data *data = i2c_get_clientdata(client);
...
static DEVICE_ATTR(ps_detection, S_IRUGO,
           apds9130_show_ps_detection, NULL);
...
static DEVICE_ATTR(ps_run_calibration, S_IRUGO | S_IWUSR| S_IWGRP,
	apds9130_show_run_calibration, apds9130_store_run_calibration);

static ssize_t apds9130_show_proximity_crosstalk(struct device *dev,
                    struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
    struct apds9130_data *data = i2c_get_clientdata(client);
...
static DEVICE_ATTR(ps_cal_result, S_IRUGO | S_IWUSR | S_IWGRP,
         apds9130_show_proximity_crosstalk, NULL);
...
#else
	err = apds9130_set_control(client, APDS9130_PDRVIE_50MA|APDS9130_PRX_IR_DIOD|APDS9130_PGAIN_2X);
#endif
	if (err < 0) return err;
...
static int apds9130_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	printk("[ProximitySensor] __set_enable : 1 \n");
    } else {
        apds9130_set_enable(client, 0);
	printk("[ProximitySensor] __set_ennable : 2 \n");
    }
