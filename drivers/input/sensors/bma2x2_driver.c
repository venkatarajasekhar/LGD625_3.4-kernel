...
#ifdef BMA2X2_ACCEL_CALIBRATION
#define CALDAT_OFFSET_X		"/sns/bmc_cal/offset_x.dat"
#define CALDAT_OFFSET_Y		"/sns/bmc_cal/offset_y.dat"
#define CALDAT_OFFSET_Z		"/sns/bmc_cal/offset_z.dat"
#endif
...
#define ACCEL_TEST_SUCCESS		0x000	/*OK*/
#define ACCEL_TEST_FAIL			0x001	/*Fail*/
#define ACCEL_TEST_NOT_READY	0x002	/*Not ready*/
#define ACCEL_TEST_DATA_FAIL	0x005	/*Get data failed*/
#define ACCEL_TEST_UNSTABLE		0x006	/*Unstable*/
#define ACCEL_TEST_VAR_ZERO		0x006	/*Zero variance*/
#define ACCEL_TEST_VAR_HIGH		0x007	/*High Variance*/
#define ACCEL_TEST_BIAS_HIGH	0x008	/*High bias*/
#define ACCEL_TEST_BIAS_X		0x010	/*X axis high bais*/
#define ACCEL_TEST_BIAS_Y		0x020	/*Y axis high bias*/
#define ACCEL_TEST_BIAS_Z		0x040	/*Z axis high bias*/
#define ACCEL_TEST_BIAS			0x080	/*High bias special error*/
#define ACCEL_TEST_SELF_X		0x100	/*X axis self test fail*/
#define ACCEL_TEST_SELF_Y		0x200	/*Y axis self test fail*/
#define ACCEL_TEST_SELF_Z		0x400	/*Z axis self test fail*/
#define ACCEL_TEST_SELF			0x800	/*Self test special error*/
...
#define ACCEL_NUM_OF_SAMPLES		64
#define ACCEL_VARIANCE_THRESHOLD		(int64_t)(FX_CONV((FX_FLTTOFIX_Q16(0.382)),16,32))
#define ACCEL_BIAS_THRESHOLD_X		FX_FLTTOFIX_Q16(1.952)
#define ACCEL_BIAS_THRESHOLD_Y		FX_FLTTOFIX_Q16(1.952)
#define ACCEL_BIAS_THRESHOLD_Z		FX_FLTTOFIX_Q16(2.526)
#endif
		...
		int1sel)
{
	...
	case 7:
		...
		data = BMA2X2_SET_BITSLICE(data, BMA2X2_EN_INT1_PAD_SLO_NO_MOT, state);
		comres = bma2x2_smbus_write_byte(client,
					BMA2X2_EN_INT1_PAD_SLO_NO_MOT__REG, &data);
		break;
	default:
		...
		int2sel)
{
	...
	case 7:
		...
		data = BMA2X2_SET_BITSLICE(data, BMA2X2_EN_INT2_PAD_SLO_NO_MOT, state);
		comres = bma2x2_smbus_write_byte(client,
					BMA2X2_EN_INT2_PAD_SLO_NO_MOT__REG, &data);
		break;
	default:
...
static int bma2x2_set_Int_Mode(struct i2c_client *client, unsigned char Mode)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_INT_MODE_SEL__REG, &data);
	data = BMA2X2_SET_BITSLICE(data, BMA2X2_INT_MODE_SEL, Mode);
	comres = bma2x2_smbus_write_byte(client, BMA2X2_INT_MODE_SEL__REG, &data);
		...
		duration)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_SLOPE_DUR__REG, &data);
	data = BMA2X2_SET_BITSLICE(data, BMA2X2_SLOPE_DUR, duration);
	comres = bma2x2_smbus_write_byte(client, BMA2X2_SLOPE_DUR__REG, &data);
		...
		*status)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_SLOPE_DURN_REG, &data);
			...
			unsigned char *status)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2x2_SLO_NO_MOT_DUR__REG, &data);
		...
		unsigned char threshold)
{
	...
	data = threshold;
	comres = bma2x2_smbus_write_byte(client, BMA2X2_SLOPE_THRES__REG, &data);
		...
		unsigned char *status)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_SLOPE_THRES_REG, &data);
	*status = data;
...
static int bma2x2_set_tap_samp(struct i2c_client *client, unsigned char samp)
{
	...
	comres = bma2x2_smbus_write_byte(client, BMA2X2_TAP_SAMPLES__REG, &data);
...
static int bma2x2_set_orient_mode(struct i2c_client *client, unsigned char mode)
{
	...
	comres = bma2x2_smbus_write_byte(client, BMA2X2_ORIENT_MODE__REG, &data);
...
#ifdef LGE_SELF_TEST
static int bma2x2_selftest_variance(struct device *dev);
		...
		const char *buf, size_t count)
{
	...
	short value2 = 0;
	short diff = 0;
	unsigned long result = ACCEL_TEST_SUCCESS;
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);

	/* variance test */
	result |= bma2x2_selftest_variance(dev);
	...
	error = strict_strtoul(buf, 10, &data);
	if (error) {
		atomic_set(&bma2x2->selftest_result, ACCEL_TEST_FAIL);
		return error;
	}
	...
	if ((bma2x2->sensor_type == BMA280_TYPE) ||
		...
		printk(KERN_INFO "%s self test: set to 4G range\n",
	...
	if ((bma2x2->sensor_type == BMA250E_TYPE) ||
		...
		printk(KERN_INFO "%s self test: set to 8G range\n",
			sensor_name[bma2x2->sensor_type]);
		/* set to 8 G range */
	...
	printk(KERN_INFO "X-axis %s\n",
		(result & ACCEL_TEST_SELF_X) ? "fail" : "pass");
	...
	printk(KERN_INFO "Y-axis %s\n",
		(result & ACCEL_TEST_SELF_Y) ? "fail" : "pass");
	...
	printk(KERN_INFO "Z-axis %s\n",
		(result & ACCEL_TEST_SELF_Z) ? "fail" : "pass");
	printk(KERN_INFO "4G range self test result: %lx\n", result);

	/* self test for bma254 */
	if ((bma2x2->sensor_type == BMA255_TYPE) && (result > 0)) {
		printk(KERN_INFO "%s self test: set to 8G range\n",
			sensor_name[bma2x2->sensor_type]);
		/* axis self test result clear */
		result &= ~(ACCEL_TEST_SELF|
					ACCEL_TEST_SELF_X|
					ACCEL_TEST_SELF_Y|
					ACCEL_TEST_SELF_Z);
		bma2x2_soft_reset(bma2x2->bma2x2_client);
		...
		printk(KERN_INFO "Z-axis %s\n",
			(result & ACCEL_TEST_SELF_Z) ? "fail" : "pass");
		printk(KERN_INFO "8G range self test result: %lx\n", result);
	}
...
static int bma2x2_selftest_variance(struct device *dev)
{
	...
	struct bma2x2acc acc_pre = {0,0,0};
	struct acc_self_test acc_sum = {0,0,0};
	struct acc_self_test acc_sum_sq = {0,0,0};
	struct acc_self_test acc_avg = {0,0,0};
	struct acc_self_test acc_var = {0,0,0};
	int result = ACCEL_TEST_SUCCESS;
	...
	if (bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type,
			&acc_pre) < 0) {
		printk(KERN_INFO "===== accelerometer data read faild =====\n");
		/* disable sensor */
		bma2x2_set_enable(dev, 0);
		return ACCEL_TEST_DATA_FAIL;
	}

	/* ready test */
	printk(KERN_INFO "accelerometer data: %d/%d/%d\n", acc_pre.x, acc_pre.y, acc_pre.z);
	if ((acc_pre.x == 0) || (acc_pre.y == 0) || (acc_pre.z == 0)) {
		printk(KERN_INFO "===== accelerometer self test is not ready =====\n");
		/* disable sensor */
		bma2x2_set_enable(dev, 0);
		return ACCEL_TEST_NOT_READY;
	}
	mdelay(10);
	bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type, &acc);
	printk(KERN_INFO "accelerometer data: %d/%d/%d\n", acc.x, acc.y, acc.z);
	if ((abs(acc.x - acc_pre.x) == 0) &&
			(abs(acc.y - acc_pre.y) == 0) &&
			(abs(acc.z - acc_pre.z) == 0)) {
		printk(KERN_INFO "===== accelerometer self test is not ready =====\n");
		/* disable sensor */
		bma2x2_set_enable(dev, 0);
		return ACCEL_TEST_NOT_READY;
	...
	do {
		mdelay(20);
		bma2x2_read_accel_xyz(bma2x2->bma2x2_client, bma2x2->sensor_type, &acc);
		if ((abs(acc.x - acc_pre.x) > BMA2X2_SHAKING_DETECT_THRESHOLD) ||
				(abs(acc.y - acc_pre.y) > BMA2X2_SHAKING_DETECT_THRESHOLD) ||
				(abs(acc.z - acc_pre.z) > BMA2X2_SHAKING_DETECT_THRESHOLD)) {
			printk(KERN_INFO "===== shaking detected =====\n");
			/* disable sensor */
			bma2x2_set_enable(dev, 0);
			return ACCEL_TEST_UNSTABLE;
		}
		else {
			acc_pre.x = acc.x;
			acc_pre.y = acc.y;
		...
		/*average*/
		acc_sum.x += acc.x;
		acc_avg.x = div64_s64(acc_sum.x, timeout_shaking);
		acc_sum.y += acc.y;
		acc_avg.y = div64_s64(acc_sum.y, timeout_shaking);
		acc_sum.z += acc.z;
		acc_avg.z = div64_s64(acc_sum.z, timeout_shaking);
		/*variance*/
		acc_sum_sq.x += (acc.x * acc.x);
		acc_var.x = div64_s64(acc_sum_sq.x, timeout_shaking)
				- (acc_avg.x * acc_avg.x);
		acc_sum_sq.y += (acc.y * acc.y);
		acc_var.y = div64_s64(acc_sum_sq.y, timeout_shaking)
				- (acc_avg.y * acc_avg.y);
		acc_sum_sq.z += (acc.z * acc.z);
		acc_var.z = div64_s64(acc_sum_sq.z, timeout_shaking)
				- (acc_avg.z * acc_avg.z);
	} while(timeout_shaking < ACCEL_NUM_OF_SAMPLES);

	printk(KERN_INFO "average: %ld/%ld/%ld\n",
		abs(acc_avg.x), abs(acc_avg.y), abs(acc_avg.z));
	printk(KERN_INFO "variance: %lld/%lld/%lld\n",
		acc_var.x, acc_var.y, acc_var.z);
	printk(KERN_INFO "variance threshold: %lld\nbias threshold : %d/%d/%d\n",
		ACCEL_VARIANCE_THRESHOLD,
		ACCEL_BIAS_THRESHOLD_X, ACCEL_BIAS_THRESHOLD_Y, ACCEL_BIAS_THRESHOLD_Z);
	if ((acc_var.x > ACCEL_VARIANCE_THRESHOLD) ||
			(acc_var.y > ACCEL_VARIANCE_THRESHOLD) ||
			(acc_var.z > ACCEL_VARIANCE_THRESHOLD)) {
		printk(KERN_INFO "===== high variance detected =====\n");
		result |= ACCEL_TEST_VAR_HIGH;
	}
	else if ((acc_var.x == 0) ||
			(acc_var.y == 0) ||
			(acc_var.z == 0)) {
		printk(KERN_INFO "===== zero variance detected =====\n");
		result |= ACCEL_TEST_VAR_ZERO;
	}
	else if (abs(acc_avg.x) > ACCEL_BIAS_THRESHOLD_X) {
		printk(KERN_INFO "===== high bias detected at axis x =====\n");
		result |= (ACCEL_TEST_BIAS_X|ACCEL_TEST_BIAS_HIGH);
	}
	else if (abs(acc_avg.y) > ACCEL_BIAS_THRESHOLD_Y) {
		printk(KERN_INFO "===== high bias detected at axis y =====\n");
		result |= (ACCEL_TEST_BIAS_Y|ACCEL_TEST_BIAS_HIGH);
	}
	else if (abs(acc_avg.z) > ACCEL_BIAS_THRESHOLD_Z) {
		printk(KERN_INFO "===== high bias detected at axis z =====\n");
		result |= (ACCEL_TEST_BIAS_Z|ACCEL_TEST_BIAS_HIGH);
	}
		...
		const char *buf, size_t count)
{
	...
	if (bma2x2_set_offset_target(bma2x2->bma2x2_client, 1,
			(unsigned char)data) < 0)
		return -EINVAL;
		...
		const char *buf, size_t count)
{
	...
	if (bma2x2_set_offset_target(bma2x2->bma2x2_client, 2,
			(unsigned char)data) < 0)
		return -EINVAL;
		...
		const char *buf, size_t count)
{
...
#ifdef CONFIG_BMA_USE_PLATFORM_DATA
	data = GET_CALIBRATION_Z_FROM_PLACE(bma2x2->bst_pd->place);
#else
	error = strict_strtoul(buf, 10, &data);
	if (error)
		return error;
	...
	if (bma2x2_set_offset_target(bma2x2->bma2x2_client, 3,
			(unsigned char)data) < 0)
		return -EINVAL;
...
static int bma2x2_read_calibration_data(struct i2c_client *client)
{
	...
	fd_offset_x = sys_open(CALDAT_OFFSET_X, O_RDONLY, 0);
	fd_offset_y = sys_open(CALDAT_OFFSET_Y, O_RDONLY, 0);
	fd_offset_z = sys_open(CALDAT_OFFSET_Z, O_RDONLY, 0);
...
static ssize_t bma2x2_write_calibration_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2x2_data *bma2x2 = i2c_get_clientdata(client);
...
static ssize_t bma2x2_write_calibration_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int idx;
	...
	fd_offset_x = sys_open(CALDAT_OFFSET_X, O_WRONLY|O_CREAT, 0666);
	fd_offset_y = sys_open(CALDAT_OFFSET_Y, O_WRONLY|O_CREAT, 0666);
	fd_offset_z = sys_open(CALDAT_OFFSET_Z, O_WRONLY|O_CREAT, 0666);
...
static DEVICE_ATTR(run_calibration, S_IRUGO|S_IWUSR|S_IWGRP,
		bma2x2_write_calibration_show, bma2x2_write_calibration_store);
